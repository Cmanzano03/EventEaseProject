    ...
    throws ServletException, IOException
	{
		...
		else
		{
			...
			for(int i=0; i<lines/2; i++)
			{
				...
				{
					secondA[i][j - column/2] = valA[i][j];
					secondB[i][j - column/2] = valB[i][j];
				}
			}
			for(int i=lines/2; i<lines; i++)
			{
				for(int j=0;j<column/2;j++)
				{
					thirdA[i - lines/2][j] = valA[i][j];
					thirdB[i - lines/2][j] = valB[i][j];
				}
			}
			...
			for(int i=lines/2; i<lines; i++)
			{
				for(int j=column/2;j<column;j++)
				{
					forthA[i - lines/2][j - column/2] = valA[i][j];
					forthB[i - lines/2][j - column/2] = valB[i][j];
				}
			}
